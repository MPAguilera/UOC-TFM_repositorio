...
#Get the  count file aand the annot file
...
## Chek that the names are in the corrcect order WSU, U2032, SUDHL6 and in each one 0,6 and 24 h
orden_names <- c()
cell_lines <- c("linea1", "linea2", "linea3")
time <- c("DMSO", "_6H_", "_24H_")
for (i in 1:length(cell_lines)){
  for (j in 1:length(time)){
  orden_names <- append(orden_names, resumen$SampleID[grep (paste0(cell_lines[i], "*.*", time[j]), resumen$SampleID)])
  }
}

resumen <-resumen[match (orden_names, resumen$SampleID),]
...
#################### - Save for the discrete files - ##########################
# Get the table with only the relevant genes (normalized by cpm)
archivo = "Timecourse_cell_lines_10%_filter_discrete.gct"
num_rows <- nrow(da)
num_cols <- ncol(da) - 2 
